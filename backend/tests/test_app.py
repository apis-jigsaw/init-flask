...
def test_categories_index(app, client):
    response = client.get('/categories')
    json_response = json.loads(response.data)

    assert len(json_response) == 2
    assert json_response[0]['name'] == 'Pizza'
    assert json_response[1]['name'] == 'Italian'
    assert list(json_response[0].keys()) == ['id', 'name']

def test_categories_show(app, client):
    response = client.get('/categories')
    json_response = json.loads(response.data)
    last_record_id = json_response[-1]['id']

    response = client.get(f'/categories/{last_record_id}')
    json_response = json.loads(response.data)
    assert json_response['name'] == 'Italian'